...
static gboolean
icon_name_changed (GtkWidget *widget, GdkEvent  *event, gpointer user_data)
{
    const gchar *text;
    CommandApplet *command_applet;
    ...
    if (g_strcmp0(command_applet->filename, text) == 0) {
        return TRUE;
    }
    ...
    g_settings_set_string (command_applet->settings, ICON_NAME_KEY, text);
    return TRUE;
}
...
command_settings_callback (GtkAction *action, CommandApplet *command_applet)
{
    ...
    g_settings_bind (command_applet->settings, ICON_NAME_KEY, GET_WIDGET ("icon_entry"), "text", G_SETTINGS_BIND_GET_NO_CHANGES);
    ...
    gtk_builder_add_callback_symbols (builder,
                                      ...
                                      "on_icon_entry_focus_out_event", G_CALLBACK (icon_name_changed),
                                      NULL);
...
settings_icon_changed (GSettings *settings, gchar *key, CommandApplet *command_applet)
{
   // GError *error = NULL;
    ...
    if (strlen (filename) == 0 || g_strcmp0(command_applet->filename, filename) == 0)
        return;
...
command_applet_fill (MatePanelApplet* applet)
{
    ...
    buf = gdk_pixbuf_scale_simple (buf, 20, 20, GDK_INTERP_BILINEAR);
