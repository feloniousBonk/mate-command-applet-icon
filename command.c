...
settings_icon_changed (GSettings *settings, gchar *key, CommandApplet *command_applet)
{
    ...
    if (command_applet->image_chooser) {
        ...
        path_gsettings = get_image_path (command_applet);
        path_chooser = gtk_file_chooser_get_filename (GTK_FILE_CHOOSER (command_applet->image_chooser));
        if (strcmp (path_gsettings, path_chooser))
            gtk_file_chooser_set_filename (GTK_FILE_CHOOSER (command_applet->image_chooser), path_gsettings);
